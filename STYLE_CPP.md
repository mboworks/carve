...
- **`clang-tidy`** with [`.clang-tidy`](.clang-tidy) runs against the compilation
  database generated by `bazel run //:refresh_compile_commands` using the maintained
  ...
  `WarningsAsErrors` makes findings fail the dedicated CI gate, as do database generation
  and tool discovery errors. Never apply clang-tidy fixes automatically. Local runs default
  to at most two workers and reserve a CPU when possible; `--jobs N` overrides the bound.
  Interruption terminates active workers and prevents queued launches. CI sets its worker
  count explicitly. See [infrastructure guidance](docs/infrastructure.md). The enabled set is
...
- **Enforce the annotations**: compile first-party code with clang's `-Wthread-safety`
  (with `-Werror` an unguarded access becomes a build failure), and run the Linux
  ThreadSanitizer CI job as the complementary runtime race detector. ASan/LSan/UBSan
  and Linux MSan cover the other sanitizer classes. LLVM-linking targets carry `no_san`
