...
class UntestedLibrariesTest(unittest.TestCase):
    def test_a_library_with_a_local_test_is_covered(self):
        self.assertEqual(cclt.untested_libraries(_LIB + _test_rule(), "carve/example"), [])
    ...
    def test_a_fully_qualified_label_to_the_same_package_counts(self):
        ...
        text = _LIB + _test_rule(dep="//carve/example:glob_cc")
        self.assertEqual(cclt.untested_libraries(text, "carve/example"), [])

    def test_a_library_with_no_test_at_all_is_reported(self):
        self.assertEqual(cclt.untested_libraries(_LIB, "carve/example"), [(1, "glob_cc")])
    ...
    def test_a_test_in_another_package_does_not_count(self):
        ...
        text = _LIB + _test_rule(dep="//carve/example:glob_cc")
        self.assertEqual(cclt.untested_libraries(text, "carve/other"), [(1, "glob_cc")])
    ...
    def test_transitive_coverage_does_not_count(self):
        ...
        self.assertEqual(cclt.untested_libraries(text, "carve/example"), [(1, "inner_cc")])
    ...
    def test_a_non_test_rule_depending_on_it_does_not_count(self):
        ...
        # is not a unit test of it.
        text = _LIB + 'cc_binary(\n    name = "glob",\n    deps = [":glob_cc"],\n)\n'
        self.assertEqual(cclt.untested_libraries(text, "carve/example"), [(1, "glob_cc")])
    ...
    def test_any_rule_kind_ending_in_test_counts(self):
        ...
        self.assertEqual(cclt.untested_libraries(_LIB + _test_rule(kind="diff_test"), "carve/example"), [])

    def test_several_libraries_are_each_checked(self):
        text = _LIB + 'cc_library(\n    name = "other_cc",\n)\n' + _test_rule()
        self.assertEqual(cclt.untested_libraries(text, "carve/example"), [(5, "other_cc")])
...
class CheckTest(unittest.TestCase):
    ...
    def test_the_message_names_the_file_line_and_library(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            build = self._write(root, "carve/example/BUILD.bazel", _LIB)
            problems = cclt.check([build], root)
        self.assertEqual(
            problems,
            ["carve/example/BUILD.bazel:1: cc_library 'glob_cc' has no test in its package depending on it"],
        )

    def test_an_allowlisted_library_is_permitted(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            build = self._write(root, "rules/testdata/BUILD.bazel", 'cc_library(\n    name = "lib_cc",\n)\n')
    ...
    def test_the_allowlist_entry_records_a_reason(self):
        ...
        self.assertIn("rules/testdata:lib_cc", cclt._ALLOWLIST)
        self.assertIn("aspect-analysis fixture", cclt._ALLOWLIST["rules/testdata:lib_cc"])
