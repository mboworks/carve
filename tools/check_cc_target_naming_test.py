...
class CheckTest(unittest.TestCase):
    def test_the_message_names_the_file_line_and_the_suggested_rename(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            (root / "carve/example").mkdir(parents=True)
            build = root / "carve/example" / "BUILD.bazel"
            build.write_text(_BAD)
            problems = cctn.check([build], root)
        self.assertEqual(
            problems,
            ["carve/example/BUILD.bazel:2: cc_library 'regex_backend' should be named 'regex_backend_cc'"],
        )
...
class FindBuildFilesTest(unittest.TestCase):
    def test_generated_and_vendored_trees_are_skipped(self):
        ...
        self.assertEqual(found, ["BUILD.bazel", "carve/example/BUILD.bazel"])
